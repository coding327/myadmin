...
8. MySearch组件的添加功能，添加需要弹框，有一个嵌套表单的对话框组件
   1. 创建对话框组件,注册并使用
   2. elementUI里面找到合适的嵌套表单的对话框组件，代码复制过来，默认不显示弹框，改为true方便操作，
   3. 兄弟通信使用事件中心最合适，点击添加控制对话框显示和隐藏【自带了确认，取消，关闭有一个值再控制和改变，直接使用事件中心改这个值实现联动】
